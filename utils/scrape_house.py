...
def parse_property(url: str, properties: List[Property]) -> Property:
    """Scrapes data from given url (from https://www.immoweb.be) into a Property object and adds it to the given list"""

    # Try to request the url & turn response into soup
    try:
        ...
        if r.status_code != 200:
            ...
            try:
                ...
                new_property.amount_of_rooms = json_data["classified"]["bedroom"][
                    "count"
                ]
                ...
                if json_data["classified"]["outdoor"]["garden"]["surface"] != "":
                    ...
                    new_property.garden_area = json_data["classified"]["outdoor"][
                        "garden"
                    ]["surface"]
                else:
                ...
                if (
                    json_data["classified"]["wellnessEquipment"]["hasSwimmingPool"]
                    == "true"
                ):
                ...
                new_property.building_state = json_data["classified"]["building"][
                    "condition"
                ]
                ...
                try:
                    if json_data["building"]["facadeCount"] != None:
                        new_property.amount_of_facades = json_data["building"][
                            "facadeCount"
                        ]
                except TypeError:
